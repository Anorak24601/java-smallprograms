...
public class FakeLotto implements ItemListener, ActionListener, Runnable {
	
	Lotto gui;
	Thread playing;
	
	public FakeLotto(Lotto in) {
		gui = in;
	}
	...
	public void run() {
		...
		while (playing == thisThread) {
			...
			for (int i = 0; i < 6; i++) {
				int lottoNum;
				do {
					lottoNum = (int)Math.floor(Math.random()*50 + 1);
				} while (numberGone(lottoNum, gui.winners, i));
				gui.winners[i].setText("" + lottoNum);
